  ...
  actions: {
    ...
    async authenticate(values) {
      ...
      try {
        ...
        if (doc.exists) {
          const totalCards = doc.data().totalCards
          cardStore.totalCards = totalCards

          const totalCreatedCards = doc.data().totalCreatedCards
          cardStore.totalCreatedCards = totalCreatedCards

          const cards = doc.data().cards
          cardStore.cards = Object.values(cards)
          cardStore.cards.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp))

          const categories = doc.data().categories
          cardStore.categories = Object.values(categories)
        }
      } catch (error) {
