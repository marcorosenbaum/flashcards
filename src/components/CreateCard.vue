<template>
  ...
  <div v-show="store.cardInputOpen">
    <textarea
      ...
      class="w-full h-20 border-2 border-black"
    ...
    <div v-show="createCategory">
      <input
        ...
        class="w-1/5 h-10 border-2 border-black"
    ...
    <button v-on:click="onCancel" class="border-2 border-black">Cancel</button>
  </div>
</template>
    ...
    onSave() {
      if (this.inputCardCategory && this.inputCardText) {
        ...
        if (Object.keys(this.store.cardToEdit).length === 0) {
          ...
          this.saveCardToFirebase(this.inputCardText, this.inputCardCategory, timestamp)
        } else
          this.saveCardToFirebase(
            ...
            this.updateCardInStore(this.inputCardText, this.inputCardCategory),
            (this.inputCardText = '')
        this.inputCardCategory = ''
      }
